...
/**
 * https://www.codewars.com/kata/52742f58faf5485cae000b9a/train/scala
 *
...
object HumanTime {
  ...
  private def excludeUnits(timeUnit: String, numPerUnit: Int): Option[String] = {
    val seconds = secondsRemain
    secondsRemain = secondsRemain / numPerUnit

    seconds % numPerUnit match {
      case x if x == 1 => Some(s"$x $timeUnit")
      case x if x > 1 => Some(s"$x ${timeUnit}s")
      case _ => None
    }
  }
  ...
  def formatDuration(seconds: Int): String = {
    ...
    if (seconds == 0) result = Seq("now")
    else {
      result ++= excludeUnits("second", 60)
      result ++= excludeUnits("minute", 60)
      result ++= excludeUnits("hour", 24)
      result ++= excludeUnits("day", 365)
      result ++= excludeUnits("year", 100)
    }

    if (result.length < 2) result.mkString
    else s"${result.reverse.init.mkString(", ")} and ${result.head}"
  }
}
